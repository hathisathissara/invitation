...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body { background: var(--parchment); font-family: 'Inter', sans-serif; color: var(--ink); min-height: 100vh; overflow-x: hidden; }
        ::selection { background: var(--plum-light); color: var(--ink); }
        .reveal { opacity: 0; }

        /* Preview banner */
        .preview-bar { background: var(--plum-dark); color: var(--parchment); text-align: center; padding: 10px 20px; font-family: 'Inter', sans-serif; font-size: 0.8rem; letter-spacing: 0.5px; position: sticky; top: 0; z-index: 200; display: flex; align-items: center; justify-content: center; gap: 12px; }
        .preview-bar a { color: var(--blush); text-decoration: underline; text-underline-offset: 3px; }

        /* ======= HERO ======= */
        .hero { position: relative; min-height: 100vh; background: linear-gradient(180deg, #fdf2f5, #fbeef1 55%, var(--parchment)); display: flex; align-items: center; justify-content: center; overflow: hidden; padding: 90px 20px 70px; }
        #hero-canvas { position: fixed; inset: 0; width: 100vw; height: 100vh; display: block; z-index: 1; pointer-events: none; }
        .hero-card { position: relative; z-index: 2; max-width: 600px; width: 100%; text-align: center; padding: 54px 42px; background: rgba(255, 255, 255, 0.52); border: 1px solid rgba(196,122,148,0.35); border-radius: 32px; backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); box-shadow: 0 30px 70px rgba(161,88,115,0.16); }
        .monogram-ring { width: 90px; height: 90px; margin: 0 auto 20px; border-radius: 50%; border: 1.5px dashed var(--plum); display: flex; align-items: center; justify-content: center; font-family: 'Fraunces', serif; font-size: 1.6rem; font-weight: 600; letter-spacing: 1px; color: var(--plum-dark); background: rgba(196,122,148,0.08); }
        .eyebrow { font-size: 0.7rem; font-weight: 600; letter-spacing: 3px; text-transform: uppercase; color: var(--sage-dark); margin-bottom: 12px; }
        .guest-line { font-family: 'Cormorant Garamond', serif; font-style: italic; font-weight: 500; font-size: clamp(1.1rem, 2.2vw, 1.4rem); color: var(--ink-mid); margin-bottom: 18px; }
        .reserved-note { margin: 0 auto 22px; background: rgba(196,122,148,0.1); border: 1px solid rgba(196,122,148,0.35); padding: 9px 18px; border-radius: 40px; display: inline-flex; align-items: center; gap: 10px; color: var(--plum-dark); font-size: 0.82rem; font-weight: 600; }
        .reserved-note i { color: var(--plum); width: 16px; }
        .couple-title { font-family: 'Fraunces', serif; font-weight: 600; font-size: clamp(2.3rem, 6vw, 3.6rem); line-height: 1.1; color: var(--ink); margin-bottom: 20px; }
        .couple-title .amp { display: block; font-family: 'Cormorant Garamond', serif; font-style: italic; font-weight: 500; font-size: 0.48em; color: var(--plum); margin: 4px 0; }
        .date-chip { display: inline-flex; flex-direction: column; gap: 3px; border: 1px solid rgba(196,122,148,0.3); border-radius: 16px; padding: 12px 24px; width: fit-content; margin: 0 auto; background: rgba(255,255,255,0.5); }
        ...
        .hero-venue { margin-top: 18px; font-family: 'Inter', sans-serif; font-size: 0.92rem; font-weight: 500; color: var(--sage-dark); display: flex; align-items: center; justify-content: center; gap: 8px; }
        .hero-venue i { color: var(--plum); }
        .scroll-cue { position: absolute; bottom: 26px; left: 50%; transform: translateX(-50%); z-index: 2; color: var(--plum-dark); font-size: 0.65rem; letter-spacing: 2px; text-transform: uppercase; display: flex; flex-direction: column; align-items: center; gap: 8px; opacity: 0.75; }
        ...
        /* ======= COUNTDOWN ======= */
        .countdown-section { background: rgba(196,122,148,0.08); border-radius: 30px; max-width: 640px; margin: 34px auto; padding: 30px 20px; text-align: center; }
        ...
        .time-unit { text-align: center; min-width: 74px; padding: 14px 6px; border-radius: 16px; background: rgba(255,255,255,0.6); border: 1px solid rgba(196,122,148,0.2); }
        ...
        /* Love story */
        .letter-card { background: var(--white); border: 2px dashed rgba(196,122,148,0.25); border-radius: 26px; padding: 42px 38px; position: relative; font-family: 'Cormorant Garamond', serif; font-style: italic; font-size: 1.25rem; line-height: 1.85; color: var(--ink-mid); box-shadow: 0 20px 50px rgba(161,88,115,0.08); }
        .letter-card::before { content: '\1F33F'; position: absolute; top: -18px; left: 34px; font-size: 2rem; background: var(--parchment); padding: 0 8px; }
        ...
        .tl-marker { position: absolute; left: -74px; top: 0; width: 60px; height: 60px; border-radius: 40% 60% 55% 45% / 55% 45% 60% 40%; background: linear-gradient(150deg, var(--plum), var(--plum-dark)); color: var(--parchment); display: flex; flex-direction: column; align-items: center; justify-content: center; font-family: 'Fraunces', serif; line-height: 1.1; box-shadow: 0 8px 22px rgba(161,88,115,0.25); }
        ...
        .btn-map { display: inline-flex; align-items: center; gap: 6px; background: linear-gradient(135deg, var(--plum), var(--plum-dark)); color: white; text-decoration: none; padding: 9px 18px; border-radius: 40px; font-size: 0.78rem; font-weight: 700; transition: all 0.2s; border: none; cursor: pointer; }
        .btn-map:hover { filter: brightness(1.08); color: white; transform: translateY(-1px); }
        .btn-cal { display: inline-flex; align-items: center; gap: 6px; background: transparent; border: 1px solid var(--parchment-border); color: var(--ink-mid); text-decoration: none; padding: 9px 18px; border-radius: 40px; font-size: 0.78rem; font-weight: 500; cursor: pointer; transition: all 0.2s; }
        ...
        .carousel-track { display: flex; gap: 26px; overflow-x: auto; scroll-snap-type: x mandatory; padding: 20px 50px 26px; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
        .carousel-track::-webkit-scrollbar { display: none; }
        .polaroid { flex: 0 0 auto; scroll-snap-align: center; width: 168px; height: 168px; border-radius: 50%; overflow: hidden; cursor: pointer; border: 4px solid var(--white); box-shadow: 0 14px 30px rgba(161,88,115,0.22); transition: transform 0.3s; position: relative; }
        .polaroid:hover { transform: scale(1.07); }
        .polaroid img { width: 100%; height: 100%; object-fit: cover; display: block; }
        .car-btn { position: absolute; top: 50%; transform: translateY(-50%); width: 44px; height: 44px; border-radius: 50%; background: var(--white); border: 1px solid var(--parchment-border); color: var(--plum-dark); display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 8px 20px rgba(161,88,115,0.15); z-index: 3; transition: all 0.2s; }
        ...
        .lightbox-close { position: absolute; top: 20px; right: 24px; color: var(--parchment); font-size: 1.8rem; cursor: pointer; }
        ...
        .guest-gallery-grid { display: flex; flex-wrap: nowrap; overflow-x: auto; gap: 15px; padding: 10px 0; scroll-behavior: smooth; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
        .guest-gallery-grid::-webkit-scrollbar { display: none; }
        .gg-item { flex: 0 0 auto; width: 200px; height: 200px; border-radius: 50%; overflow: hidden; position: relative; cursor: pointer; border: 3px solid var(--white); box-shadow: 0 10px 24px rgba(161,88,115,0.18); transition: transform 0.3s; }
        .gg-item img { width: 100%; height: 100%; object-fit: cover; display: block; }
        .gg-item:hover { transform: translateY(-4px) scale(1.03); }
        ...
        .rsvp-note { width: 100%; background: var(--parchment); border: 1px solid var(--parchment-border); border-radius: 16px; padding: 13px 15px; font-family: 'Inter', sans-serif; font-size: 0.86rem; color: var(--ink-mid); resize: none; outline: none; margin-bottom: 16px; }
...
<div class="preview-bar">
    ...
    <strong>{{ t('preview_mode_label') }}</strong> — {{ t('preview_mode_note') }}
    <a href="{{ route('dashboard') }}">{{ t('back_to_dashboard') }}</a>
</div>
@endif

@if (session('status'))
<div style="max-width:720px; margin:20px auto; padding:0 20px;">
    <div style="padding: 13px 18px; border-radius: 12px; font-size: 0.87rem; background: rgba(34,197,94,0.1); border:1px solid rgba(34,197,94,0.25); color: #16a34a; display:flex; align-items:center; gap:10px;">
...
<div class="hero" {!! $hero_style !!}>
    <div class="hero-card reveal">
        ...
        <p class="guest-line">{{ t('hero_dear') }} {{ $guestName }},</p>
        ...
        <div class="date-chip">
            <span class="lbl">{{ t('hero_getting_married') }}</span>
    ...
    <div class="scroll-cue"><span class="line"></span>{{ t('hero_scroll_cue') }}</div>
</div>

<!-- COUNTDOWN -->
<div class="countdown-section">
    <p class="countdown-label">{{ t('countdown_label') }}</p>
    <div class="countdown" id="countdown">
        <div class="time-unit"><span class="time-value" id="cd-days">00</span><span class="time-label">{{ t('cd_days') }}</span></div>
        <div class="time-unit"><span class="time-value" id="cd-hours">00</span><span class="time-label">{{ t('cd_hours') }}</span></div>
        <div class="time-unit"><span class="time-value" id="cd-mins">00</span><span class="time-label">{{ t('cd_mins') }}</span></div>
        <div class="time-unit"><span class="time-value" id="cd-secs">00</span><span class="time-label">{{ t('cd_secs') }}</span></div>
    </div>
</div>
...
<div class="invitation-body container py-4">
    ...
    <div class="section-head reveal">
        <span class="tag">{{ t('love_story_tag') }}</span>
        <h2>{!! t('love_story_title') !!}</h2>
    </div>
    <div class="letter-card reveal">
        {!! nl2br(e($wedding->love_story)) !!}
    </div>
    @endif

    <!-- PROGRAMME -->
    <div class="section-head reveal">
        <span class="tag">{{ t('programme_tag') }}</span>
        <h2>{!! t('programme_title') !!}</h2>
    </div>
                ...
                <div class="tl-card">
                    ...
                    <div class="tl-actions">
                        ...
                        <a href="{{ $ev->google_map_link }}" target="_blank" class="btn-map" rel="noopener">
                            <i class="fas fa-directions"></i> {{ t('get_directions') }}
                        </a>
                        @endif
                        <div class="cal-dropdown">
                            <button class="btn-cal" onclick="toggleCal(this)">
                                <i class="fas fa-calendar-plus"></i> {{ t('add_to_calendar') }}
                            </button>
                            <div class="cal-menu">
                                <a href="{{ $ev_gcal }}" target="_blank" rel="noopener"><i class="fab fa-google" style="color:#4285f4;"></i> Google Calendar</a>
                                <a href="{{ $icsLink }}" download><i class="fab fa-apple" style="color:#555;"></i> Apple Calendar</a>
    ...
    @else
        <p style="text-align:center; color:var(--ink-light); font-style:italic; padding:30px 0;">{{ t('event_details_soon') }}</p>
    @endif

    <!-- GALLERY -->
    @if ($galleryImages->count() > 0)
    <div class="section-head reveal">
        <span class="tag">{{ t('gallery_tag') }}</span>
    ...
    <div class="carousel-wrap reveal">
        ...
        <div class="carousel-track-outer">
            <div class="carousel-track" id="gallery-grid">
                ...
                <div class="polaroid" onclick="openLightbox('{{ asset($img->image_path) }}')">
...
@if ($hasGuestGallery)
    ...
    <h2 class="section-heading">{!! t('guest_gallery_title') !!}</h2>
    ...
    <div style="max-width:760px; margin:0 auto 30px; background: var(--cream-2); border: 2px dashed var(--plum); border-radius: 20px; padding: 30px 20px; text-align: center;">
        @if ($isPreview)
            <p class="text-muted small"><i class="fas fa-lock"></i> {{ t('upload_disabled_preview') }}</p>
        @else
            <i class="fas fa-camera-retro" style="font-size: 2.2rem; color: var(--plum); margin-bottom: 12px; display: block;"></i>
            <h5 class="fw-bold" style="font-family:'Inter', sans-serif; font-size: 0.95rem; color: var(--ink); margin-bottom: 6px;">{{ t('share_photo_heading') }}</h5>
            <p class="text-muted" style="font-size: 0.8rem; margin-bottom: 15px;">{{ t('share_photo_desc') }}</p>
            ...
            <button type="button" class="btn-map" onclick="document.getElementById('guest-image-input').click()" style="border: none; cursor: pointer;">
                <i class="fas fa-cloud-upload-alt"></i> {{ t('upload_wedding_photo_btn') }}
            </button>
    ...
    <div class="guest-gallery-grid" id="guest-gallery-grid" style="max-width:760px; margin:0 auto 30px; padding-left:20px; padding-right:20px;">
        @if ($guestImages->count() > 0)
            @foreach ($guestImages as $g_img)
            <div class="gg-item reveal" onclick="openLightbox('{{ asset($g_img->image_path) }}')" style="border-color: #22c55e;">
...
<div class="rsvp-section">
    <div class="rsvp-card text-center shadow rounded-5 p-4">
        ...
        <div class="rsvp-form-side">
            <h2 class="rsvp-title">{{ t('rsvp_title') }}</h2>
            <p class="rsvp-subtitle">{{ t('rsvp_subtitle') }}</p>
            
            @if ($currentGuest && $currentGuest->seats_reserved > 0)
            <div class="reserved-note" style="margin-bottom: 20px;">
                <i class="fas fa-chair"></i>
                <span>{{ t('seats_reserved') }}: <strong>{{ $currentGuest->seats_reserved }}</strong></span>
            </div>
            @endif
            ...
            <form method="POST" action="{{ route('invitation.rsvp', $wedding->slug) }}">
                ...
                <button type="submit" class="btn-rsvp-submit">{{ t('rsvp_submit') }}</button>
            </form>
        </div>
    </div>
</div>
...
function tick() {
    ...
    if (dist < 0) {
        document.getElementById('countdown').innerHTML = '<p class="just-married-msg">{{ t("just_married") }}</p>';
        return;
    }
