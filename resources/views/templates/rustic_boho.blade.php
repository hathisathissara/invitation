...
$rsvp_success = session('rsvp_success', false);
...
<!-- HERO HEADER -->
<div class="hero-header" {!! $hero_style !!}>
    <canvas id="hero3d-canvas" aria-hidden="true"></canvas>
    <div class="hero-content reveal" id="invitation-main">
        <span class="guest-greeting-tag">Together with our families</span>
        <div class="guest-name-display">Dear {{ $guestName }},</div>
        ...
        <div class="hero-date-area">
            <p class="hero-getting-married">Getting Married</p>
            <p class="hero-date">{{ t_date($wedding->wedding_date) }}</p>
...
@endphp
    <h2 class="section-heading reveal">Love Story</h2>
    <p class="section-sub">Our Journey</p>
    <div class="story-timeline" id="story-timeline">
        ...
        @foreach($story_chapters as $i => $chapter)
@php
            $side  = $i % 2 === 0 ? 'left' : 'right';
            $icon  = $story_icons[$i % count($story_icons)];
            $label = $story_labels[$i % count($story_labels)];
@endphp
        <div class="story-milestone {{ $side }}" data-side="{{ $side }}">
            <div class="story-milestone-marker"><i class="fas {{ $icon }}"></i></div>
            <div class="story-milestone-card">
                <span class="story-milestone-label">{{ $label }}</span>
                <p>{!! nl2br($chapter) !!}</p>
            </div>
        </div>
        @endforeach
    ...
    <!-- EVENTS / PROGRAMME -->
    <h2 class="section-heading reveal">Programme</h2>
    <p class="section-sub">Event Details</p>
...
@endphp
            <div class="event-card col reveal">
                ...
                <div class="event-meta">
                    <div class="event-meta-item"><i class="far fa-calendar"></i><span>{{ t_date($ev->event_date_time) }}</span></div>
                    <div class="event-meta-item"><i class="far fa-clock"></i><span>{{ t_time($ev->event_date_time) }}</span></div>
                    <div class="event-meta-item"><i class="fas fa-map-marker-alt"></i><span>{{ $ev->location_name }}</span></div>
                </div>
                ...
                <div class="venue-map-wrap">
                    <div class="venue-map-card tilt-card">
                        <iframe
                            src="https://www.google.com/maps?q={{ urlencode($ev->location_name) }}&output=embed"
                            loading="lazy"
                            title="Map preview for {{ $ev->location_name }}"
    ...
    <h2 class="section-heading reveal">Our Gallery</h2>
    <p class="section-sub">Memories</p>

    <div class="sweet-slideshow reveal" id="sweet-slideshow">
        <div class="sweet-slideshow-track" id="sweet-slideshow-track">
            @foreach($galleryImages as $img)
            <div class="sweet-slide" onclick="openLightbox('{{ asset($img->image_path) }}')">
...
<!-- =====================================================================
    ...
    <h2 class="section-heading reveal">Shared Moments</h2>
...
<!-- =====================================================================
         ...
         $guestMessages (array of ['guest_name' => ..., 'message' => ...]),
         mirroring the existing $hasGuestGallery / $guestImages pattern.
         ...
         ===================================================================== -->
    @if(isset($has_memory_wall) && $has_memory_wall)
    <div class="invitation-body" style="padding-top:20px;">
    ...
    <div class="memory-wall-grid" id="memory-wall-grid">
        @if($guestMessages->count() > 0)
            @foreach($guestMessages as $gm)
            <div class="memory-card reveal">
                <p>&ldquo;{!! nl2br($gm->message) !!}&rdquo;</p>
                <div class="memory-author">— {{ $gm->guest_name }}</div>
            </div>
            @endforeach
        @else
    ...
    document.getElementById('memory-submit-btn')?.addEventListener('click', function() {
        ...
        fetch("{{ route('invitation.message', $wedding->slug) }}", {
            method: 'POST',
...
const RSVP_JUST_SUCCEEDED = {{ $rsvp_success ? 'true' : 'false' }};
